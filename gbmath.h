...
// Accurate approximation of sine function using signed GBfloats
uGBfloat GBfloat_sin(uGBfloat angle) {
    GBfloat angleIndex = unsigned_angle_to_index(angle);
    uint16_t index = uGBfloatToInt16(angleIndex);
...
//convert signed GBfloat degrees to radians
uGBfloat GBfloatDegreesToRadians(uGBfloat degrees) {
    return GBfloatDivide(GBfloatMultiply(degrees, UGBFLOAT_PI), floatToGBfloat(180, 0));
}
//convert signed GBfloat radians to degrees
uGBfloat GBfloatRadiansToDegrees(uGBfloat radians) {
